    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'remove_successor':
                ...
                header('Location: succession-planning.php?tab=
                critical-roles');
                exit;
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <?php if ($isAdmin): ?>
                    <button onclick="showAddHighPotentialModal()" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        <i class="fas fa-plus"></i>
                        <span>Add High Potential Employee</span>
                    </button>
                <?php endif; ?>
            </div>
        ...
        <form id="addSuccessorForm" method="POST" action="" class="p-6">
            ...
            <div>
                ...
                <select name="employee_id" id="successor_employee_select" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    foreach ($employees as $emp): 
                    ?>
                        <option value="<?php echo $emp['id']; ?>"><?php echo htmlspecialchars($emp['full_name'] . ' - ' . $emp['department'] . ' (' . $emp['position'] . ')'); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mt-6 flex justify-end gap-3">
...
<!-- Add High Potential Employee Modal -->
<div id="addHighPotentialModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <h3 class="text-xl font-bold text-gray-900">Add High Potential Employee</h3>
            <button onclick="closeAddHighPotentialModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        <form id="addHighPotentialForm" method="POST" action="" class="p-6">
            <input type="hidden" name="action" value="add_high_potential">
            <div class="space-y-4">
                <div>
                ...
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Current Role</label>
                    <input type="text" name="current_role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-gray-700 text-sm font-medium mb-2">Years of Service</label>
                        <input type="number" min="0" name="years_of_service" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-gray-700 text-sm font-medium mb-2">Performance Rating (1–5)</label>
                        <input type="number" step="0.1" min="1" max="5" name="performance_rating" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-gray-700 text-sm font-medium mb-2">Potential Score (0–100)</label>
                        <input type="number" min="0" max="100" name="potential_score" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Target Role</label>
                    <input type="text" name="target_role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Development Areas (comma-separated)</label>
                    <textarea name="development_areas" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"></textarea>
                </div>
            </div>
            <div class="mt-6 flex justify-end gap-3">
                <button type="button" onclick="closeAddHighPotentialModal()" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                    Cancel
                </button>
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                    Add High Potential
